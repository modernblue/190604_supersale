...
if (sexStatus == "men") {
	$(".isWomen").hide();
	$(".isMen").show();
	$(".sex__list .sex__men").addClass("active");
} else {
	$(".isMen").hide();
	$(".isWomen").show();
	$(".sex__list .sex__women").addClass("active");
}
...
$(window).load(function() {
	...
	$.ajax({
		type: "GET",
		url: 'https://mb-rakuten-sale.firebaseio.com/marathon_0501.json',
		data: {event: 'marathon_190511'},
		timeout: 5000,
		dataType: 'json'
	}).done(function(data){
		console.log(data);
		//$("#ttl").text(JSON.stringify(data));
	}).fail(function(data){
...
$(function() {
	$(".sex__list .item").on('click', function() {

		//console.log($(this));
		$(".sex__list .item").removeClass("active");
